...
- **Motion blur is ON by default** for every visual and camera layer (the compiler sets `motion_blur: true`) — pass `"motion_blur": false` on a layer to disable it. For visible blur on fast moves, raise the comp's `shutter_angle` (default 0.5; 1–1.3 reads more cinematic).
