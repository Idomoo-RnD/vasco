...
- **Rich spans (typography)** — `"styles"` is a list of per-character-range overrides, each `{ "start": <char index>, "length": <chars>, ... }`. **Confirmed to render:** `color` (hex), `size`, `tracking`, `leading`, `shift`, and a per-span `font` (`font` optional — defaults to the layer font). Example:
  ...
  - ⚠️ **Non-ASCII + multiple spans currently fails cloud render** (exporter error 3000). If the text contains a multi-byte character (`×`, `€`, `–`, accents, CJK, emoji…) and a span boundary falls on/after it, the exporter splits mid-UTF-8 and the render crashes — even though `validate`/`compile` pass locally. Until the exporter is fixed: use **one span over the whole string**, substitute an ASCII equivalent (`x` for `×`, `-` for `–`), or keep multi-byte characters out of split spans. `validate`/`compile` print a ⚠ warning when they detect this combination.
