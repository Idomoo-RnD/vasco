...
- **Text** — assume values longer *and* shorter than your sample. Generous box width; keep `shrink:true` with a sensible `min_size`; choose alignment deliberately (left-aligned grows right; centred grows both ways). Never split one personal value across hand-positioned layers. Put a realistic **long** sample in the scene so the layout is proven against the hard case.
