...
- **Known exporter bug (validate warns):** **Text in ≥2 sub-comps + images.** If a text layer lives in two or more sub-compositions while the scene uses image/video, the render fails (3000). Put the text in the **main** comp over image-only sub-comps, or keep text in at most one sub-comp.
