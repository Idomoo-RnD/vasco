...
- **Unknown keys are NOT ignored — they break the compile.** Any key that isn't documented sugar is passed straight into the VASCO output, and the VASCO schema is strict (`additionalProperties: false`). So an invented or mistyped key — `z`, `zIndex`, `x`, `y`, `width`/`height` on a layer, `comment`, `id`, `label`, `radius`, `src` on a non-media layer, etc. — fails the compile with "unknown key …". Use only the documented sugar or a property that is genuinely part of raw VASCO for that object. The passthrough is an escape hatch for *real* VASCO properties, not a free-form bag.
- **Verify before you render.** Run `strata validate scene.json` (free, offline, no auth) — it schema-checks the compiled VASCO and names any offending key/layer. Fix until it's clean, then compile/render so you never burn a render on a structural error.
